...
sfc_timestamping_profile_tab_t::sfc_timestamping_profile_tab_t() {
    uint i;

    max_entries = 2;
    for (i = 0; i < 2; i++) {
        sfc_timestamping_profile_entry_array[i] = new sfc_timestamping_profile_entry_t;
    }
}
...
sfc_timestamping_profile_tab_t::~sfc_timestamping_profile_tab_t() {
    ...
    max_entries = 2;
    for (i = 0; i < 2; i++) {
        delete sfc_timestamping_profile_entry_array[i];
    }
}

void sfc_timestamping_profile_tab_t::write_entry(_1bit *index, sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry) {
    uint i;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("sfc_timestamping_profile::write_entry() index = %d", (int)*index);
#endif
    sfc_timestamping_profile_entry_array[*index]->_VALID = sfc_timestamping_profile_entry->_VALID;
    sfc_timestamping_profile_entry_array[*index]->_INDEX = sfc_timestamping_profile_entry->_INDEX;
    sfc_timestamping_profile_entry_array[*index]->timestamping_index = sfc_timestamping_profile_entry->timestamping_index;
    sfc_timestamping_profile_entry_array[*index]->in_ts_enable = sfc_timestamping_profile_entry->in_ts_enable;
    sfc_timestamping_profile_entry_array[*index]->out_ts_valid = sfc_timestamping_profile_entry->out_ts_valid;
    sfc_timestamping_profile_entry_array[*index]->out_ts_val_sec = sfc_timestamping_profile_entry->out_ts_val_sec;
    sfc_timestamping_profile_entry_array[*index]->out_ts_val_nsec = sfc_timestamping_profile_entry->out_ts_val_nsec;
    sfc_timestamping_profile_entry_array[*index]->_LOOKUP0 = sfc_timestamping_profile_entry->_LOOKUP0;
    sfc_timestamping_profile_entry_array[*index]->_HIT_INDEX0 = sfc_timestamping_profile_entry->_HIT_INDEX0;
}

void sfc_timestamping_profile_tab_t::find_index(sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry, int *idx, int *avbl_idx, int *pidx) {
    _1bit _full_key = 0;

    *idx = -1;
    *avbl_idx = -1;
    *pidx = -1;
    _full_key = (sfc_timestamping_profile_entry->timestamping_index);
    if (sfc_timestamping_profile_entry_array[_full_key]->_VALID == 1) {
        *idx = _full_key;
    } else {
        *avbl_idx = _full_key;
    }
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("sfc_timestamping_profile::find_index() idx = %d; avbl_idx = %d; pidx = %d", *idx, *avbl_idx, *pidx);
#endif
}

int sfc_timestamping_profile_tab_t::insert_entry(sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry) {
    int idx, avbl_idx, pidx;
    _1bit index;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("sfc_timestamping_profile::insert_entry()");
#endif
    find_index(sfc_timestamping_profile_entry, &idx, &avbl_idx, &pidx);
    if(idx != -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("sfc_timestamping_profile::insert_entry() table entry in use");
#endif
        return -2;
    }
    ...
    if(avbl_idx == -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("sfc_timestamping_profile::insert_entry() table full");
#endif
        return -3;
    }

    index = (_1bit) avbl_idx;
    sfc_timestamping_profile_entry->_VALID = 1;
    write_entry(&index, sfc_timestamping_profile_entry);
    return 0;
}

int sfc_timestamping_profile_tab_t::delete_entry(sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry) {
    int idx, avbl_idx, pidx;
    _1bit index;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("sfc_timestamping_profile::delete_entry()");
#endif
    find_index(sfc_timestamping_profile_entry, &idx, &avbl_idx, &pidx);
    if(idx == -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("sfc_timestamping_profile::delete_entry() could not find entry");
#endif
        return -1;
    }

    index = (_1bit) idx;
    sfc_timestamping_profile_entry->_VALID = 0;
    write_entry(&index, sfc_timestamping_profile_entry);
    return 0;
}

int sfc_timestamping_profile_tab_t::lookup_entry(sfc_timestamping_profile_entry_t **sfc_timestamping_profile_entry) {
    sfc_timestamping_profile_entry_t *rentry;
    int idx, avbl_idx, pidx;
    _1bit index;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("sfc_timestamping_profile::lookup_entry()");
#endif
    find_index(*sfc_timestamping_profile_entry, &idx, &avbl_idx, &pidx);
    if(idx == -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("sfc_timestamping_profile::lookup_entry() could not find entry");
#endif
        return -1;
    }
    ...
    memcpy(*sfc_timestamping_profile_entry, rentry, sizeof(sfc_timestamping_profile_entry_t));
    return 0;
}

sfc_timestamping_profile_entry_t* sfc_timestamping_profile_tab_t::read_entry(_1bit *index) {
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("sfc_timestamping_profile::read_entry() index = %d", (int)*index);
#endif
    return sfc_timestamping_profile_entry_array[*index];

}

sfc_timestamping_profile_entry_t* sfc_timestamping_profile_tab_t::lookup(int _lookup_idx_, _1bit *index) {
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->normal("Looking up sfc_timestamping_profile");
#endif

    sfc_timestamping_profile_entry_array[*index]->_INDEX = *index;
    sfc_timestamping_profile_entry_array[*index]->_LOOKUP0 = 1;
    sfc_timestamping_profile_entry_array[*index]->_HIT_INDEX0(31,31) = 1;
    sfc_timestamping_profile_entry_array[*index]->_HIT_INDEX0(30,24) = SFC_TIMESTAMPING_PROFILE_ENUM;
    sfc_timestamping_profile_entry_array[*index]->_HIT_INDEX0(23,0) = *index;
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("sfc_timestamping_profile::lookup() index = %d", (int)*index);
#endif
