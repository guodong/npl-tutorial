...
typedef enum _table_enums_e_ {
    SFC_TIMESTAMPING_PROFILE_ENUM,
    SFC_EDB_PROFILE_ENUM,
    SFC_MMU_PROFILE_ENUM,
    SFC_IARB_PROFILE_ENUM,
} _table_enums_t_;

class sfc_timestamping_profile_tab_t {
public:
    sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry_array[2];
    uint max_entries;

    sfc_timestamping_profile_tab_t(void);
    ~sfc_timestamping_profile_tab_t(void);

    void write_entry(_1bit *index, sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry);
    void find_index(sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry, int *idx, int *avbl_idx, int *pidx);
    int insert_entry(sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry);
    int delete_entry(sfc_timestamping_profile_entry_t *sfc_timestamping_profile_entry);
    int lookup_entry(sfc_timestamping_profile_entry_t **sfc_timestamping_profile_entry);
    sfc_timestamping_profile_entry_t* read_entry(_1bit *index);
    sfc_timestamping_profile_entry_t* lookup(int _lookup_idx_, _1bit *index);
};
