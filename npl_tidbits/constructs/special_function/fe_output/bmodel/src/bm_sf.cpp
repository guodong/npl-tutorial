...
void _BModel::sfc_timestamping_profile__sfc_call(void *tbl_ptr, int index)
{
    sfc_timestamping_profile_entry_t *entry_ptr = (sfc_timestamping_profile_entry_t *)tbl_ptr;
    arg_addr_info_t *arg_info;
    _bits_ull_base_t *bub;
    _bits_base_t *bb;
    _1bit arg_0_val = 0;
    _1bit *arg_0 = &arg_0_val;
    _1bit arg_1_val = 0;
    _1bit *arg_1 = &arg_1_val;
    _32bits arg_2_val = 0;
    _32bits *arg_2 = &arg_2_val;
    _32bits arg_3_val = 0;
    _32bits *arg_3 = &arg_3_val;
    ...
    assert(index < sfc_timestamping_profile__bk->table_rows);

    /* load arguments */
    if (entry_ptr->in_ts_enable == NULL) {
        arg_0 = NULL;
    }
    else if (sfc_timestamping_profile__bk->idx_arg_hit(index, 0)) {
        arg_info = sfc_timestamping_profile__bk->idx_arg_get(index, 0);
        if (arg_info->size <= BIT_BASE_BOUNDRY) {
            bub = (_bits_ull_base_t*)arg_info->addr;
            *arg_0 = bub->range(arg_info->high, arg_info->low);
        } else {
        ...
        arg_0 = (_1bit*)entry_ptr->in_ts_enable;
    }

    if (entry_ptr->out_ts_valid == NULL) {
        /* output - use local variable */
    }
    else if (sfc_timestamping_profile__bk->idx_arg_hit(index, 1)) {
        arg_info = sfc_timestamping_profile__bk->idx_arg_get(index, 1);
        if (arg_info->size <= BIT_BASE_BOUNDRY) {
            bub = (_bits_ull_base_t*)arg_info->addr;
            *arg_1 = bub->range(arg_info->high, arg_info->low);
        } else {
        ...
        *arg_1 = *(_1bit*)entry_ptr->out_ts_valid;
    }

    if (entry_ptr->out_ts_val_sec == NULL) {
        /* output - use local variable */
    }
    else if (sfc_timestamping_profile__bk->idx_arg_hit(index, 2)) {
        arg_info = sfc_timestamping_profile__bk->idx_arg_get(index, 2);
        if (arg_info->size <= BIT_BASE_BOUNDRY) {
            bub = (_bits_ull_base_t*)arg_info->addr;
            *arg_2 = bub->range(arg_info->high, arg_info->low);
        } else {
        ...
        *arg_2 = *(_32bits*)entry_ptr->out_ts_val_sec;
    }

    if (entry_ptr->out_ts_val_nsec == NULL) {
        /* output - use local variable */
    }
    else if (sfc_timestamping_profile__bk->idx_arg_hit(index, 3)) {
        arg_info = sfc_timestamping_profile__bk->idx_arg_get(index, 3);
        if (arg_info->size <= BIT_BASE_BOUNDRY) {
            bub = (_bits_ull_base_t*)arg_info->addr;
            *arg_3 = bub->range(arg_info->high, arg_info->low);
        } else {
        ...
        *arg_3 = *(_32bits*)entry_ptr->out_ts_val_nsec;
    }


    /* call special function */
    sfc_timestamping_func(arg_0, arg_1, arg_2, arg_3);

    /* write output arguments */
    switch (index) {
        case (0): {
            local_bus->set_ts_valid(*arg_1, 0, 0);
            local_bus->set_ts_val_sec(*arg_2, 31, 0);
            local_bus->set_ts_val_nsec(*arg_3, 31, 0);
            break;
        }
        default: {
#ifndef NO_CONSOLE_OUTPUT
