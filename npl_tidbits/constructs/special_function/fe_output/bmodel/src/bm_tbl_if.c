...
sfc_timestamping_profile_entry_t *sfc_timestamping_profile_loc_entry = NULL;
               ...
               uint32_t *data, size_t wsize)
{
    ...
    switch (sid) {
        ...
        case SFC_TIMESTAMPING_PROFILEm:
            if (bm_obj->sfc_timestamping_profile->max_entries > (uint) dyn_info->index) {
                index1 = dyn_info->index;
                sfc_timestamping_profile_loc_entry = bm_obj->sfc_timestamping_profile->read_entry(&index1);
#ifndef NO_CONSOLE_OUTPUT
                if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                    sfc_timestamping_profile_loc_entry->print();
                }
#endif
                sfc_timestamping_profile_loc_entry->pack_data(data);
            } else {
                if (bm_obj->sfc_timestamping_profile->max_entries != (uint) dyn_info->index) {
#ifndef NO_CONSOLE_OUTPUT
                    bmi_log->error("ERROR: %s(): Table sfc_timestamping_profile, invalid index (%d)", __FUNCTION__, (uint) dyn_info->index);
#endif
                    return 1;
                } else if (typ == 3) {
                    sfc_timestamping_profile_loc_entry = new sfc_timestamping_profile_entry_t;
                    sfc_timestamping_profile_loc_entry->unpack_data(data);
                    status = bm_obj->sfc_timestamping_profile->lookup_entry(&sfc_timestamping_profile_loc_entry);
#ifndef NO_CONSOLE_OUTPUT
                    if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                        sfc_timestamping_profile_loc_entry->print();
                    }
#endif
                    sfc_timestamping_profile_loc_entry->pack_data(data);
                    delete(sfc_timestamping_profile_loc_entry);
                } else {
#ifndef NO_CONSOLE_OUTPUT
                    bmi_log->error("ERROR: %s(): Table sfc_timestamping_profile, invalid operation (%d)", __FUNCTION__, typ);
#endif
                    return 1;
                }
            }
...
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                bmi_log->normal("%s(): register_ts_ctrl:", __FUNCTION__);
                bm_obj->register_ts_ctrl->print();
            }
#endif
            field = (uint32_t)bm_obj->register_ts_ctrl->enable;
    ...
    switch (sid) {
        ...
        case SFC_TIMESTAMPING_PROFILEm:
            if(bm_obj->sfc_timestamping_profile->max_entries > (uint) dyn_info->index) {
                ...
                sfc_timestamping_profile_loc_entry = bm_obj->sfc_timestamping_profile->read_entry(&index1);
                sfc_timestamping_profile_loc_entry->unpack_data(data);
                bm_obj->sfc_timestamping_profile->write_entry(&index1, sfc_timestamping_profile_loc_entry);
#ifndef NO_CONSOLE_OUTPUT
                if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                    sfc_timestamping_profile_loc_entry->print();
                }
                ...
                sfc_timestamping_profile_loc_entry->unpack_data(data);
                if (typ == 0) {
                    status = bm_obj->sfc_timestamping_profile->insert_entry(sfc_timestamping_profile_loc_entry);
                } else if (typ == 2) {
                    status = bm_obj->sfc_timestamping_profile->delete_entry(sfc_timestamping_profile_loc_entry);
                }
#ifndef NO_CONSOLE_OUTPUT
                if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                    sfc_timestamping_profile_loc_entry->print();
        ...
        case TS_CTRLm:
            ...
            bm_obj->register_ts_ctrl->set_enable((_1bit)field, -1, -1);
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                bmi_log->normal("%s(): register_ts_ctrl:", __FUNCTION__);
                bm_obj->register_ts_ctrl->print();
...
int bm_lt_entry(int unit, bm_sid_t sid, bm_pt_dyn_info_t *dyn_info)
{
    ...
    switch (sid) {
        ...
        case SFC_TIMESTAMPING_PROFILEm:
            ...
            sfc_timestamping_profile_loc_entry = bm_obj->sfc_timestamping_profile->read_entry(&index1);
#ifndef NO_CONSOLE_OUTPUT
            bmi_log->normal("%s(): sfc_timestamping_profile[%d]:", __FUNCTION__, dyn_info->index);
            sfc_timestamping_profile_loc_entry->print();
#endif
            break;
