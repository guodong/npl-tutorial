...
void local_bus_t::set_ts_enable(_1bit value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->ts_enable: 0x" + ts_enable.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_ts_valid(_1bit value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((ts_valid(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    ts_valid(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->ts_valid: 0x" + ts_valid.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_ts_val_sec(_32bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((ts_val_sec(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    ts_val_sec(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->ts_val_sec: 0x" + ts_val_sec.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_ts_val_nsec(_32bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((ts_val_nsec(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    ts_val_nsec(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->ts_val_nsec: 0x" + ts_val_nsec.to_string(SC_HEX_US, false));
    }
#endif
}
...
void local_bus_t::set_resetval(void) {
    ...
    ts_enable = (_1bit)(0x0U);
    ts_valid = (_1bit)(0x0U);
    ts_val_sec = (_32bits)(0x0U);
    ts_val_nsec = (_32bits)(0x0U);
}
...
void local_bus_t::set_randval(void) {
    ...
    ts_enable = (_1bit)(rand());
    ts_valid = (_1bit)(rand());
    ts_val_sec = (_32bits)(rand());
    ts_val_nsec = (_32bits)(rand());
}
...
void local_bus_t::print() {
    ...
    bmi_log->normal("local_bus_t->ts_enable = 0x" + ts_enable.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->ts_valid = 0x" + ts_valid.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->ts_val_sec = 0x" + ts_val_sec.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->ts_val_nsec = 0x" + ts_val_nsec.to_string(SC_HEX_US, false));
#endif
}
...
int local_bus_t::compare(local_bus_t *ref, local_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        if (ts_valid != ref->ts_valid) {
            mismatch = 1;
        }
    } else {
        if ((ts_valid & ref_mask->ts_valid) != (ref->ts_valid & ref_mask->ts_valid)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (ts_val_sec != ref->ts_val_sec) {
            mismatch = 1;
        }
    } else {
        if ((ts_val_sec & ref_mask->ts_val_sec) != (ref->ts_val_sec & ref_mask->ts_val_sec)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (ts_val_nsec != ref->ts_val_nsec) {
            mismatch = 1;
        }
    } else {
        if ((ts_val_nsec & ref_mask->ts_val_nsec) != (ref->ts_val_nsec & ref_mask->ts_val_nsec)) {
            mismatch = 1;
        }
    }
...
void local_bus_t::deep_copy(local_bus_t *ref) {
    ...
    ts_enable = ref->ts_enable;
    ts_valid = ref->ts_valid;
    ts_val_sec = ref->ts_val_sec;
    ts_val_nsec = ref->ts_val_nsec;
}
...
void local_bus_t::unpack_data(uint32_t *_data, int offset) {
    ...
    set_ts_enable((_1bit)field, -1, -1);
    bm_field_get(_data, 1, 1, &field);
    set_ts_valid((_1bit)field, -1, -1);
    bm_field_get(_data, 2, 33, &field);
    set_ts_val_sec((_32bits)field, -1, -1);
    bm_field_get(_data, 34, 65, &field);
    set_ts_val_nsec((_32bits)field, -1, -1);
}
...
void local_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) ts_valid.range(0, 0).to_int();
    bm_field_set(_data, (offset + 1), (offset + 1), &field);
    field = (uint32_t) ts_val_sec.range(31, 0).to_int();
    bm_field_set(_data, (offset + 2), (offset + 33), &field);
    field = (uint32_t) ts_val_nsec.range(31, 0).to_int();
    bm_field_set(_data, (offset + 34), (offset + 65), &field);
}
