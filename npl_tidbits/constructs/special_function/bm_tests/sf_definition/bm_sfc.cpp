...
/* -----------------------------------------------
*  Implementation of timestamping sf in BM
* ------------------------------------------------ */
void _BModel::sfc_timestamping_func (_1bit *in_ctrl_bus__ts_enable, _1bit *out_ctrl_bus__ts_vaild, _32bits *out_ctrl_bus__ts_val_sec, _32bits *out_ctrl_bus__ts_val_nsec){
    int rv;
    struct timespec time = {0,0};

    if (in_ctrl_bus__ts_enable) {

        // Using linux system function to get the current time of the machine
        rv = clock_gettime (CLOCK_REALTIME, &time);

        if (rv == 0) {
            *out_ctrl_bus__ts_vaild = 1;
        }
        *out_ctrl_bus__ts_val_sec = (_32bits) time.tv_sec;
        *out_ctrl_bus__ts_val_nsec = (_32bits) time.tv_nsec;
    }
}
