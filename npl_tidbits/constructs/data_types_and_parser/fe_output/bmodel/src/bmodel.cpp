...
#endif
    if ((ingress_pkt->l2_hdr->l2->ethertype == 0x88A8)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
#endif
        _AUTO_PARSER_NODE_otag();
        return;
    } else {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
#endif
        if ((ingress_pkt->l2_hdr->l2->ethertype == 0x8100)) {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
            }
#endif
            _AUTO_PARSER_NODE_itag();
            return;
        } else {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
void _BModel::_AUTO_PARSER_NODE_otag(void) {
...
#endif
    if (ingress_pkt->l2_hdr->outer_tag->extract_fields(ing_pkt, &hdr_ptr, &pkt_len)!=0) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->error("ERROR: %s(): extract_fields() error", __FUNCTION__);
#endif
        return;
    }
    ingress_pkt->l2_hdr->outer_tag->_PRESENT = 1;
    ingress_pkt->l2_hdr->outer_tag->_PRESENT_CUR = 1;
    update_mid__ingress_pkt__l2_hdr__outer_tag();
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->l2_hdr->outer_tag->ethertype: 0x%s;  line: %d", ingress_pkt->l2_hdr->outer_tag->ethertype.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((ingress_pkt->l2_hdr->outer_tag->ethertype == 0x8100)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
...
#endif
        if ((ingress_pkt->l2_hdr->outer_tag->ethertype == 0x0800)) {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
            }
...
void _BModel::_AUTO_PARSER_NODE_itag(void) {
...
#endif
    if (ingress_pkt->l2_hdr->inner_tag->extract_fields(ing_pkt, &hdr_ptr, &pkt_len)!=0) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->error("ERROR: %s(): extract_fields() error", __FUNCTION__);
#endif
        return;
    }
    ingress_pkt->l2_hdr->inner_tag->_PRESENT = 1;
    ingress_pkt->l2_hdr->inner_tag->_PRESENT_CUR = 1;
    update_mid__ingress_pkt__l2_hdr__inner_tag();
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->l2_hdr->inner_tag->ethertype: 0x%s;  line: %d", ingress_pkt->l2_hdr->inner_tag->ethertype.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((ingress_pkt->l2_hdr->inner_tag->ethertype == 0x0800)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
...
void _BModel::ing_main(void) {
    ...
    printf("Src Mac: 0x%x\n", (int)ingress_pkt->l2_hdr->l2->macsa);
    printf("Dst Mac: 0x%x\n", (int)ingress_pkt->l2_hdr->l2->macda);
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->l2_hdr->inner_tag->_PRESENT: 0x%s;  line: %d", ingress_pkt->l2_hdr->inner_tag->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
        bmi_log->normal("BM_TRACE: ingress_pkt->l2_hdr->outer_tag->_PRESENT: 0x%s;  line: %d", ingress_pkt->l2_hdr->outer_tag->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
...
#endif
        printf("Packet is double tagged\n");
        printf("Outer VlanId is %d\n", (int)ingress_pkt->l2_hdr->outer_tag->vid);
        printf("Inner VlanId is %d\n", (int)ingress_pkt->l2_hdr->inner_tag->vid);
    }
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->l2_hdr->inner_tag->_PRESENT: 0x%s;  line: %d", ingress_pkt->l2_hdr->inner_tag->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
...
#endif
        printf("Packet is single outer tagged\n");
        printf("Outer VlanId is %d\n", (int)ingress_pkt->l2_hdr->outer_tag->vid);
    }
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
#endif
        printf("Packet is single inner tagged\n");
        printf("Inner VlanId is %d\n", (int)ingress_pkt->l2_hdr->inner_tag->vid);
    }
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
#endif
        printf("Source ip address is 0X%x\n", (int)ingress_pkt->l3_hdr->ipv4->sip);
        printf("Destination ip address is 0X%x\n", (int)ingress_pkt->l3_hdr->ipv4->dip);
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: ingress_pkt->l3_hdr->ipv4->option: 0x%s;  line: %d", ingress_pkt->l3_hdr->ipv4->option.to_string(SC_HEX_US, false).c_str(), __LINE__);
        }
#endif
...
#endif
            printf("This is a packet with IP options\n");
        }
    }
