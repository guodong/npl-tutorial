...
void local_bus_t::set_zero_sa(_1bit value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((zero_sa(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    zero_sa(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->zero_sa: 0x" + zero_sa.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_zero_da(_1bit value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((zero_da(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    zero_da(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->zero_da: 0x" + zero_da.to_string(SC_HEX_US, false));
    }
#endif
}
...
void local_bus_t::print() {
    ...
    bmi_log->normal("local_bus_t->zero_sa = 0x" + zero_sa.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->zero_da = 0x" + zero_da.to_string(SC_HEX_US, false));
#endif
}
...
int local_bus_t::compare(local_bus_t *ref, local_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        if (zero_sa != ref->zero_sa) {
            mismatch = 1;
        }
    } else {
        if ((zero_sa & ref_mask->zero_sa) != (ref->zero_sa & ref_mask->zero_sa)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (zero_da != ref->zero_da) {
            mismatch = 1;
        }
    } else {
        if ((zero_da & ref_mask->zero_da) != (ref->zero_da & ref_mask->zero_da)) {
            mismatch = 1;
        }
    }
...
void local_bus_t::unpack_data(uint32_t *_data, int offset) {
    ...
    set_zero_sa((_1bit)field, -1, -1);
    bm_field_get(_data, 1, 1, &field);
    set_zero_da((_1bit)field, -1, -1);
}
...
void local_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) zero_sa.range(0, 0).to_int();
    bm_field_set(_data, (offset + 0), (offset + 0), &field);
    field = (uint32_t) zero_da.range(0, 0).to_int();
