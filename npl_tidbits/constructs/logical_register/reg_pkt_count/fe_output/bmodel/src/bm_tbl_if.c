               ...
               uint32_t *data, size_t wsize)
{
    ...
    switch (sid) {
        ...
        case ING_PKT_COUNTERm:
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                bmi_log->normal("%s(): register_ing_pkt_counter:", __FUNCTION__);
                bm_obj->register_ing_pkt_counter->print();
            }
#endif
            field = (uint32_t)bm_obj->register_ing_pkt_counter->pkt_count;
                ...
                uint32_t *data)
{
    ...
    switch (sid) {
        ...
        case ING_PKT_COUNTERm:
            ...
            bm_obj->register_ing_pkt_counter->set_pkt_count((_32bits)field, -1, -1);
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_DEBUG_NZ) {
                bmi_log->normal("%s(): register_ing_pkt_counter:", __FUNCTION__);
                bm_obj->register_ing_pkt_counter->print();
