...
#endif
    if (ingress_pkt->grp->l2->extract_fields(ing_pkt, &hdr_ptr, &pkt_len)!=0) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->error("ERROR: %s(): extract_fields() error", __FUNCTION__);
#endif
        return;
    }
    ingress_pkt->grp->l2->_PRESENT = 1;
    ingress_pkt->grp->l2->_PRESENT_CUR = 1;
    update_mid__ingress_pkt__grp__l2();
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->grp->l2->ethertype: 0x%s;  line: %d", ingress_pkt->grp->l2->ethertype.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((ingress_pkt->grp->l2->ethertype == 0x8100)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
#endif
        _AUTO_PARSER_NODE_outer_tag();
        return;
    } else {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
#endif
        if ((ingress_pkt->grp->l2->ethertype == 0x9100)) {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
            }
#endif
            _AUTO_PARSER_NODE_inner_tag();
            return;
        } else {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
void _BModel::_AUTO_PARSER_NODE_outer_tag(void) {
...
#endif
    if (ingress_pkt->grp->outer_tag->extract_fields(ing_pkt, &hdr_ptr, &pkt_len)!=0) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->error("ERROR: %s(): extract_fields() error", __FUNCTION__);
#endif
        return;
    }
    ingress_pkt->grp->outer_tag->_PRESENT = 1;
    ingress_pkt->grp->outer_tag->_PRESENT_CUR = 1;
    update_mid__ingress_pkt__grp__outer_tag();
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->grp->outer_tag->ethertype: 0x%s;  line: %d", ingress_pkt->grp->outer_tag->ethertype.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((ingress_pkt->grp->outer_tag->ethertype == 0x9100)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
...
void _BModel::_AUTO_PARSER_NODE_inner_tag(void) {
...
#endif
    if (ingress_pkt->grp->inner_tag->extract_fields(ing_pkt, &hdr_ptr, &pkt_len)!=0) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->error("ERROR: %s(): extract_fields() error", __FUNCTION__);
#endif
        return;
    }
    ingress_pkt->grp->inner_tag->_PRESENT = 1;
    ingress_pkt->grp->inner_tag->_PRESENT_CUR = 1;
    update_mid__ingress_pkt__grp__inner_tag();
    _AUTO_PARSER_NODE_ingress();
    return;
...
#endif
    set_bits_16 = 0x8100; 
    local_bus->set_tpid(set_bits_16, 15, 0);
    egress_pkt->grp->set_l2(ingress_pkt->grp->l2);
    update_mid__egress_pkt__grp__l2();
    egress_pkt->grp->set_outer_tag(ingress_pkt->grp->outer_tag);
    update_mid__egress_pkt__grp__outer_tag();
    egress_pkt->grp->set_inner_tag(ingress_pkt->grp->inner_tag);
    update_mid__egress_pkt__grp__inner_tag();
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: egress_pkt->grp->outer_tag->_PRESENT: 0x%s;  line: %d", egress_pkt->grp->outer_tag->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((egress_pkt->grp->outer_tag->_PRESENT != 0)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
#endif
        printf("Remove outer tag, and replace L2 ethertype\n");
        //replace_header_field(&egress_pkt->grp->l2->ethertype, &egress_pkt->grp->outer_tag->ethertype);
        if (egress_pkt->grp->l2->_PRESENT_CUR == 1) {
            egress_pkt->grp->l2->ethertype = egress_pkt->grp->outer_tag->ethertype;
            egress_pkt->grp->l2->pack();
            egress_pkt->grp->l2->update_pkt(egr_pkt);
        } else {
#ifndef NO_CONSOLE_OUTPUT
            bmi_log->error("ERROR: %s() Header egress_pkt->grp->l2 not present", __FUNCTION__);
#endif
        }
        delete_header(egr_pkt, egress_pkt->grp->outer_tag);
    } else {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        ...
        printf("Add outer tag, replace L2 ethertype\n");
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: egress_pkt->grp->inner_tag->_PRESENT: 0x%s;  line: %d", egress_pkt->grp->inner_tag->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
        }
#endif
        if ((egress_pkt->grp->inner_tag->_PRESENT != 0)) {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
            }
        ...
        egress_pkt->grp->outer_tag->pack();
        add_header(egr_pkt, egress_pkt, egress_pkt->grp->outer_tag);
        //replace_header_field(&egress_pkt->grp->l2->ethertype, &local_bus->tpid);
        if (egress_pkt->grp->l2->_PRESENT_CUR == 1) {
            egress_pkt->grp->l2->ethertype = local_bus->tpid;
            egress_pkt->grp->l2->pack();
            egress_pkt->grp->l2->update_pkt(egr_pkt);
        } else {
#ifndef NO_CONSOLE_OUTPUT
...
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->grp->outer_tag->_PRESENT: 0x%s;  line: %d", ingress_pkt->grp->outer_tag->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((ingress_pkt->grp->outer_tag->_PRESENT != 0)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
...
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: ingress_pkt->grp->inner_tag->_PRESENT: 0x%s;  line: %d", ingress_pkt->grp->inner_tag->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
        }
#endif
        if ((ingress_pkt->grp->inner_tag->_PRESENT != 0)) {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
            }
#endif
            printf("Ingress packet is double tagged. \n");
        } else {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
...
#endif
            printf("Ingress packet is single outer tagged. \n");
        }
    } else {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
#endif
            printf("Ingress packet is single inner tagged. \n");
        } else {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
...
#endif
            printf("Ingress packet is untagged. \n");
        }
    }
