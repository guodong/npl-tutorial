...
void local_bus_t::set_tpid(_16bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((tpid(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    tpid(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->tpid: 0x" + tpid.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_vid(_12bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->vid: 0x" + vid.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_pcp(_3bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((pcp(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    pcp(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->pcp: 0x" + pcp.to_string(SC_HEX_US, false));
    }
#endif
}
...
void local_bus_t::print() {
    ...
    bmi_log->normal("local_bus_t->tpid = 0x" + tpid.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->vid = 0x" + vid.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->pcp = 0x" + pcp.to_string(SC_HEX_US, false));
#endif
}
...
int local_bus_t::compare(local_bus_t *ref, local_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        ...
        if ((tpid & ref_mask->tpid) != (ref->tpid & ref_mask->tpid)) {
            mismatch = 1;
        }
    }
    ...
    if (ref_mask == NULL) {
        ...
        if ((pcp & ref_mask->pcp) != (ref->pcp & ref_mask->pcp)) {
            mismatch = 1;
        }
    }
...
void local_bus_t::unpack_data(uint32_t *_data, int offset) {
    ...
    bm_field_get(_data, 0, 15, &field);
    set_tpid((_16bits)field, -1, -1);
    bm_field_get(_data, 16, 27, &field);
    set_vid((_12bits)field, -1, -1);
    bm_field_get(_data, 28, 30, &field);
    set_pcp((_3bits)field, -1, -1);
}
...
void local_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) tpid.range(15, 0).to_int();
    bm_field_set(_data, (offset + 0), (offset + 15), &field);
    field = (uint32_t) vid.range(11, 0).to_int();
    bm_field_set(_data, (offset + 16), (offset + 27), &field);
    field = (uint32_t) pcp.range(2, 0).to_int();
    bm_field_set(_data, (offset + 28), (offset + 30), &field);
}
