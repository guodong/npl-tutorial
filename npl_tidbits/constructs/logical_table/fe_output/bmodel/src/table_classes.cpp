...
vlan_table_tab_t::vlan_table_tab_t() {
    uint i;

    max_entries = 4096;
    for (i = 0; i < 4096; i++) {
        vlan_table_entry_array[i] = new vlan_table_entry_t;
    }
}
...
vlan_table_tab_t::~vlan_table_tab_t() {
    ...
    max_entries = 4096;
    for (i = 0; i < 4096; i++) {
        delete vlan_table_entry_array[i];
    }
}

void vlan_table_tab_t::write_entry(_12bits *index, vlan_table_entry_t *vlan_table_entry) {
    uint i;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("vlan_table::write_entry() index = %d", (int)*index);
#endif
    vlan_table_entry_array[*index]->_VALID = vlan_table_entry->_VALID;
    vlan_table_entry_array[*index]->_INDEX = vlan_table_entry->_INDEX;
    vlan_table_entry_array[*index]->vid = vlan_table_entry->vid;
    vlan_table_entry_array[*index]->vlan_membership_bitmap = vlan_table_entry->vlan_membership_bitmap;
    vlan_table_entry_array[*index]->_LOOKUP0 = vlan_table_entry->_LOOKUP0;
    vlan_table_entry_array[*index]->_HIT_INDEX0 = vlan_table_entry->_HIT_INDEX0;
}

void vlan_table_tab_t::find_index(vlan_table_entry_t *vlan_table_entry, int *idx, int *avbl_idx, int *pidx) {
    _12bits _full_key = 0;

    *idx = -1;
    *avbl_idx = -1;
    *pidx = -1;
    _full_key = (vlan_table_entry->vid);
    if (vlan_table_entry_array[_full_key]->_VALID == 1) {
        *idx = _full_key;
    } else {
        *avbl_idx = _full_key;
    }
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("vlan_table::find_index() idx = %d; avbl_idx = %d; pidx = %d", *idx, *avbl_idx, *pidx);
#endif
}

int vlan_table_tab_t::insert_entry(vlan_table_entry_t *vlan_table_entry) {
    int idx, avbl_idx, pidx;
    _12bits index;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("vlan_table::insert_entry()");
#endif
    find_index(vlan_table_entry, &idx, &avbl_idx, &pidx);
    if(idx != -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("vlan_table::insert_entry() table entry in use");
#endif
        return -2;
    }
    ...
    if(avbl_idx == -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("vlan_table::insert_entry() table full");
#endif
        return -3;
    }

    index = (_12bits) avbl_idx;
    vlan_table_entry->_VALID = 1;
    write_entry(&index, vlan_table_entry);
    return 0;
}

int vlan_table_tab_t::delete_entry(vlan_table_entry_t *vlan_table_entry) {
    int idx, avbl_idx, pidx;
    _12bits index;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("vlan_table::delete_entry()");
#endif
    find_index(vlan_table_entry, &idx, &avbl_idx, &pidx);
    if(idx == -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("vlan_table::delete_entry() could not find entry");
#endif
        return -1;
    }

    index = (_12bits) idx;
    vlan_table_entry->_VALID = 0;
    write_entry(&index, vlan_table_entry);
    return 0;
}

int vlan_table_tab_t::lookup_entry(vlan_table_entry_t **vlan_table_entry) {
    vlan_table_entry_t *rentry;
    int idx, avbl_idx, pidx;
    _12bits index;

#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("vlan_table::lookup_entry()");
#endif
    find_index(*vlan_table_entry, &idx, &avbl_idx, &pidx);
    if(idx == -1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->normal("vlan_table::lookup_entry() could not find entry");
#endif
        return -1;
    }
    ...
    index = (_12bits) idx;
    rentry = read_entry(&index);
    memcpy(*vlan_table_entry, rentry, sizeof(vlan_table_entry_t));
    return 0;
}

vlan_table_entry_t* vlan_table_tab_t::read_entry(_12bits *index) {
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("vlan_table::read_entry() index = %d", (int)*index);
#endif
    return vlan_table_entry_array[*index];

}

vlan_table_entry_t* vlan_table_tab_t::lookup(int _lookup_idx_, _12bits *index) {
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->normal("Looking up vlan_table");
#endif

    vlan_table_entry_array[*index]->_INDEX = *index;
    vlan_table_entry_array[*index]->_LOOKUP0 = 1;
    vlan_table_entry_array[*index]->_HIT_INDEX0(31,31) = 1;
    vlan_table_entry_array[*index]->_HIT_INDEX0(30,24) = VLAN_TABLE_ENUM;
    vlan_table_entry_array[*index]->_HIT_INDEX0(23,0) = *index;
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->debug("vlan_table::lookup() index = %d", (int)*index);
#endif
