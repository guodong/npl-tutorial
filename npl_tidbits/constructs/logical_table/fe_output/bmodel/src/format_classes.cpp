...
void obj_bus_t::set_vid(_12bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((vid(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }
...
#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: obj_bus_t->vid: 0x" + vid.to_string(SC_HEX_US, false));
    }
#endif
}

void obj_bus_t::set_vlan_membership_bitmap(_128bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((vlan_membership_bitmap(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }
...
#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: obj_bus_t->vlan_membership_bitmap: 0x" + vlan_membership_bitmap.to_string(SC_HEX_US, false));
    }
#endif
}

void obj_bus_t::set_drop_vector(_16bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: obj_bus_t->drop_vector: 0x" + drop_vector.to_string(SC_HEX_US, false));
    }
#endif
}
...
void obj_bus_t::set_randval(void) {
    ...
    dst_port = (_7bits)(rand());
    vid = (_12bits)(rand());
    vlan_membership_bitmap = (_128bits)(rand());
    drop_vector = (_16bits)(rand());
}
...
void obj_bus_t::print() {
    ...
    bmi_log->normal("obj_bus_t->vid = 0x" + vid.to_string(SC_HEX_US, false));
    bmi_log->normal("obj_bus_t->vlan_membership_bitmap = 0x" + vlan_membership_bitmap.to_string(SC_HEX_US, false));
    bmi_log->normal("obj_bus_t->drop_vector = 0x" + drop_vector.to_string(SC_HEX_US, false));
#endif
}
...
int obj_bus_t::compare(obj_bus_t *ref, obj_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        ...
        if ((vid & ref_mask->vid) != (ref->vid & ref_mask->vid)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (vlan_membership_bitmap != ref->vlan_membership_bitmap) {
            mismatch = 1;
        }
    } else {
        if ((vlan_membership_bitmap & ref_mask->vlan_membership_bitmap) != (ref->vlan_membership_bitmap & ref_mask->vlan_membership_bitmap)) {
            mismatch = 1;
        }
    }
...
void obj_bus_t::deep_copy(obj_bus_t *ref) {
    ...
    dst_port = ref->dst_port;
    vid = ref->vid;
    vlan_membership_bitmap = ref->vlan_membership_bitmap;
    drop_vector = ref->drop_vector;
}
...
void obj_bus_t::unpack_data(uint32_t *_data, int offset) {
    ...
    set_dst_port((_7bits)field, -1, -1);
    bm_field_get(_data, 24, 35, &field);
    set_vid((_12bits)field, -1, -1);
    bm_field_get(_data, 36, 163, &field);
    set_vlan_membership_bitmap((_128bits)field, -1, -1);
    bm_field_get(_data, 164, 179, &field);
    set_drop_vector((_16bits)field, -1, -1);
}
...
void obj_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) vid.range(11, 0).to_int();
    bm_field_set(_data, (offset + 24), (offset + 35), &field);
    field = (uint32_t) vlan_membership_bitmap.range(31, 0).to_int();
    bm_field_set(_data, (offset + 36), (offset + 67), &field);
    field = (uint32_t) vlan_membership_bitmap.range(31, 0).to_int();
    bm_field_set(_data, (offset + 36), (offset + 67), &field);
    field = (uint32_t) vlan_membership_bitmap.range(31, 0).to_int();
    bm_field_set(_data, (offset + 36), (offset + 67), &field);
    ...
    bm_field_set(_data, (offset + 36), (offset + 67), &field);
    field = (uint32_t) drop_vector.range(15, 0).to_int();
    bm_field_set(_data, (offset + 164), (offset + 179), &field);
}
