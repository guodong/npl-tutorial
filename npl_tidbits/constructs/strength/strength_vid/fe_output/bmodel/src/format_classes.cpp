...
void local_bus_t::set_pkt_protocol(_1bit value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((pkt_protocol(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }
...
#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->pkt_protocol: 0x" + pkt_protocol.to_string(SC_HEX_US, false));
    }
#endif
}
...
void local_bus_t::print() {
    ...
    bmi_log->normal("local_bus_t->pkt_protocol = 0x" + pkt_protocol.to_string(SC_HEX_US, false));
...
int local_bus_t::compare(local_bus_t *ref, local_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        if (pkt_protocol != ref->pkt_protocol) {
            mismatch = 1;
        }
    } else {
        if ((pkt_protocol & ref_mask->pkt_protocol) != (ref->pkt_protocol & ref_mask->pkt_protocol)) {
            mismatch = 1;
        }
    }
...
void local_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) pkt_protocol.range(0, 0).to_int();
    bm_field_set(_data, (offset + 0), (offset + 0), &field);
    field = (uint32_t) vid.range(11, 0).to_int();
    bm_field_set(_data, (offset + 1), (offset + 12), &field);
}
