...
int ipv4_t::extract_fields(_8bits *pkt, _32bits *hdr_ptr, _32bits *pkt_len) {
    ...
    for (i = 0; i < 4; i++) {
        if (i != 3) {
            sa |= (((_32bits)(pkt[*hdr_ptr + byte_ptr + i]) << (8 * (4 - i - 1))));
        } else {
            sa |= (((_32bits)(pkt[*hdr_ptr + byte_ptr + i]) >> 0));
        }
    }
    ...
    for (i = 0; i < 4; i++) {
        if (i != 3) {
            da |= (((_32bits)(pkt[*hdr_ptr + byte_ptr + i]) << (8 * (4 - i - 1))));
        } else {
            da |= (((_32bits)(pkt[*hdr_ptr + byte_ptr + i]) >> 0));
        }
    }
...
void ipv4_t::pack() {
    ...
    for (i = 0; i < 4; i++) {
        if (i != 3) {
            hdr_bytes[byte_ptr + i] |= (_8bits)(((sa) >> (8 * (4 - i - 1))));
        } else {
            hdr_bytes[byte_ptr + i] |= (_8bits)(((sa) << 0));
        }
    }
    byte_ptr = 16;
    for (i = 0; i < 4; i++) {
        if (i != 3) {
            hdr_bytes[byte_ptr + i] |= (_8bits)(((da) >> (8 * (4 - i - 1))));
        } else {
            hdr_bytes[byte_ptr + i] |= (_8bits)(((da) << 0));
        }
    }
    _loc_hdr_len = (hdr_len * 4 + 0);
    if (_loc_hdr_len < 20) {
...
void ipv4_t::print() {
    ...
    bmi_log->normal("ipv4_t->sa = 0x" + sa.to_string(SC_HEX_US, false));
    bmi_log->normal("ipv4_t->da = 0x" + da.to_string(SC_HEX_US, false));
...
void group_t::set_ipv4(ipv4_t *ref) {
    uint i;
    ipv4->deep_copy(ref);
}
