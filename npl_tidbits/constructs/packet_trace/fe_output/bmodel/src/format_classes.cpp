...
void local_bus_t::set_ipv4_with_option(_1bit value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((ipv4_with_option(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    ipv4_with_option(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->ipv4_with_option: 0x" + ipv4_with_option.to_string(SC_HEX_US, false));
    }
#endif
}
...
void local_bus_t::set_resetval(void) {
    ...
    ipv4_with_option = (_1bit)(0x0U);
}

void local_bus_t::set_randval(void) {
    uint i;

    ipv4_with_option = (_1bit)(rand());
}
...
void local_bus_t::print() {
    ...
    bmi_log->normal("local_bus_t->ipv4_with_option = 0x" + ipv4_with_option.to_string(SC_HEX_US, false));
#endif
}
...
int local_bus_t::compare(local_bus_t *ref, local_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        if (ipv4_with_option != ref->ipv4_with_option) {
            mismatch = 1;
        }
    } else {
        if ((ipv4_with_option & ref_mask->ipv4_with_option) != (ref->ipv4_with_option & ref_mask->ipv4_with_option)) {
            mismatch = 1;
        }
    }
...
void local_bus_t::deep_copy(local_bus_t *ref) {
    ...
    ipv4_with_option = ref->ipv4_with_option;
}
...
void local_bus_t::unpack_data(uint32_t *_data, int offset) {
    ...
    set_ipv4_with_option((_1bit)field, -1, -1);
}
...
void local_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) ipv4_with_option.range(0, 0).to_int();
