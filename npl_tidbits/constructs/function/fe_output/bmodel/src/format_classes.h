...
public:
    _32bits hash_sip;
    _32bits hash_dip;
    _8bits hash_protocol;
    _16bits hash_src_port;
    _16bits hash_dst_port;
    _8bits hash_val;

    local_bus_t(void);
    ~local_bus_t(void);

    void set_hash_sip(_32bits, int, int);
    void set_hash_dip(_32bits, int, int);
    void set_hash_protocol(_8bits, int, int);
    void set_hash_src_port(_16bits, int, int);
    void set_hash_dst_port(_16bits, int, int);
    void set_hash_val(_8bits, int, int);

    void set_resetval(void);
    void set_randval(void);
    void print(void);
    int compare(local_bus_t *ref, local_bus_t *ref_mask);
    void deep_copy(local_bus_t *ref);
    void unpack_data(uint32_t *_data, int offset = 0);
    void pack_data(uint32_t *_data, int offset = 0);
    int minbit();
    int maxbit();
