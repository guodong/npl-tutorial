...
void local_bus_t::set_hash_sip(_32bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((hash_sip(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    hash_sip(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->hash_sip: 0x" + hash_sip.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_hash_dip(_32bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((hash_dip(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    hash_dip(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->hash_dip: 0x" + hash_dip.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_hash_protocol(_8bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((hash_protocol(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    hash_protocol(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->hash_protocol: 0x" + hash_protocol.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_hash_src_port(_16bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((hash_src_port(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    hash_src_port(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->hash_src_port: 0x" + hash_src_port.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_hash_dst_port(_16bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((hash_dst_port(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    hash_dst_port(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->hash_dst_port: 0x" + hash_dst_port.to_string(SC_HEX_US, false));
    }
#endif
}

void local_bus_t::set_hash_val(_8bits value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((hash_val(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    hash_val(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->hash_val: 0x" + hash_val.to_string(SC_HEX_US, false));
    }
#endif
}
...
void local_bus_t::set_resetval(void) {
    ...
    hash_sip = (_32bits)(0x0U);
    hash_dip = (_32bits)(0x0U);
    hash_protocol = (_8bits)(0x0U);
    hash_src_port = (_16bits)(0x0U);
    hash_dst_port = (_16bits)(0x0U);
    hash_val = (_8bits)(0x0U);
}

void local_bus_t::set_randval(void) {
    uint i;

    hash_sip = (_32bits)(rand());
    hash_dip = (_32bits)(rand());
    hash_protocol = (_8bits)(rand());
    hash_src_port = (_16bits)(rand());
    hash_dst_port = (_16bits)(rand());
    hash_val = (_8bits)(rand());
}
...
void local_bus_t::print() {
    ...
    bmi_log->normal("local_bus_t->hash_sip = 0x" + hash_sip.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->hash_dip = 0x" + hash_dip.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->hash_protocol = 0x" + hash_protocol.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->hash_src_port = 0x" + hash_src_port.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->hash_dst_port = 0x" + hash_dst_port.to_string(SC_HEX_US, false));
    bmi_log->normal("local_bus_t->hash_val = 0x" + hash_val.to_string(SC_HEX_US, false));
#endif
}
...
int local_bus_t::compare(local_bus_t *ref, local_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        if (hash_sip != ref->hash_sip) {
            mismatch = 1;
        }
    } else {
        if ((hash_sip & ref_mask->hash_sip) != (ref->hash_sip & ref_mask->hash_sip)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (hash_dip != ref->hash_dip) {
            mismatch = 1;
        }
    } else {
        if ((hash_dip & ref_mask->hash_dip) != (ref->hash_dip & ref_mask->hash_dip)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (hash_protocol != ref->hash_protocol) {
            mismatch = 1;
        }
    } else {
        if ((hash_protocol & ref_mask->hash_protocol) != (ref->hash_protocol & ref_mask->hash_protocol)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (hash_src_port != ref->hash_src_port) {
            mismatch = 1;
        }
    } else {
        if ((hash_src_port & ref_mask->hash_src_port) != (ref->hash_src_port & ref_mask->hash_src_port)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (hash_dst_port != ref->hash_dst_port) {
            mismatch = 1;
        }
    } else {
        if ((hash_dst_port & ref_mask->hash_dst_port) != (ref->hash_dst_port & ref_mask->hash_dst_port)) {
            mismatch = 1;
        }
    }

    if (ref_mask == NULL) {
        if (hash_val != ref->hash_val) {
            mismatch = 1;
        }
    } else {
        if ((hash_val & ref_mask->hash_val) != (ref->hash_val & ref_mask->hash_val)) {
            mismatch = 1;
        }
    }
...
void local_bus_t::deep_copy(local_bus_t *ref) {
    ...
    hash_sip = ref->hash_sip;
    hash_dip = ref->hash_dip;
    hash_protocol = ref->hash_protocol;
    hash_src_port = ref->hash_src_port;
    hash_dst_port = ref->hash_dst_port;
    hash_val = ref->hash_val;
}
...
void local_bus_t::unpack_data(uint32_t *_data, int offset) {
    ...
    set_hash_sip((_32bits)field, -1, -1);
    bm_field_get(_data, 32, 63, &field);
    set_hash_dip((_32bits)field, -1, -1);
    bm_field_get(_data, 64, 71, &field);
    set_hash_protocol((_8bits)field, -1, -1);
    bm_field_get(_data, 72, 87, &field);
    set_hash_src_port((_16bits)field, -1, -1);
    bm_field_get(_data, 88, 103, &field);
    set_hash_dst_port((_16bits)field, -1, -1);
    bm_field_get(_data, 104, 111, &field);
    set_hash_val((_8bits)field, -1, -1);
}
...
void local_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) hash_sip.range(31, 0).to_int();
    bm_field_set(_data, (offset + 0), (offset + 31), &field);
    field = (uint32_t) hash_dip.range(31, 0).to_int();
    bm_field_set(_data, (offset + 32), (offset + 63), &field);
    field = (uint32_t) hash_protocol.range(7, 0).to_int();
    bm_field_set(_data, (offset + 64), (offset + 71), &field);
    field = (uint32_t) hash_src_port.range(15, 0).to_int();
    bm_field_set(_data, (offset + 72), (offset + 87), &field);
    field = (uint32_t) hash_dst_port.range(15, 0).to_int();
    bm_field_set(_data, (offset + 88), (offset + 103), &field);
    field = (uint32_t) hash_val.range(7, 0).to_int();
    bm_field_set(_data, (offset + 104), (offset + 111), &field);
}
