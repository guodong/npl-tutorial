...
#endif
    if ((ingress_pkt->group2->ipv4->protocol == 0x06)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
#endif
        _AUTO_PARSER_NODE_tcp();
        return;
    } else {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
void _BModel::_AUTO_PARSER_NODE_tcp(void) {
...
#endif
    if (ingress_pkt->group2->tcp->extract_fields(ing_pkt, &hdr_ptr, &pkt_len)!=0) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->error("ERROR: %s(): extract_fields() error", __FUNCTION__);
#endif
        return;
    }
    ingress_pkt->group2->tcp->_PRESENT = 1;
    ingress_pkt->group2->tcp->_PRESENT_CUR = 1;
    update_mid__ingress_pkt__group2__tcp();
    _AUTO_PARSER_NODE_ingress();
    return;
...
void _BModel::do_ip_hash_calc(void) {
    _32bits set_bits_32;
    _8bits set_bits_8;
    _16bits set_bits_16;
...
#endif
    set_bits_32 = 0; 
    local_bus->set_hash_sip(set_bits_32, 31, 0);
    set_bits_32 = 0; 
    local_bus->set_hash_dip(set_bits_32, 31, 0);
    set_bits_8 = 0; 
    local_bus->set_hash_protocol(set_bits_8, 7, 0);
    set_bits_16 = 0; 
    local_bus->set_hash_src_port(set_bits_16, 15, 0);
    set_bits_16 = 0; 
    local_bus->set_hash_dst_port(set_bits_16, 15, 0);
    set_bits_8 = 0x0; 
    local_bus->set_hash_val(set_bits_8, 7, 0);
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: ingress_pkt->group2->ipv4->_PRESENT: 0x%s;  line: %d", ingress_pkt->group2->ipv4->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((ingress_pkt->group2->ipv4->_PRESENT != 0)) {
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
        }
        ...
        set_bits_32 = ingress_pkt->group2->ipv4->sip; 
        local_bus->set_hash_sip(set_bits_32, 31, 0);
        set_bits_32 = ingress_pkt->group2->ipv4->dip; 
        local_bus->set_hash_dip(set_bits_32, 31, 0);
        set_bits_8 = ingress_pkt->group2->ipv4->protocol; 
        local_bus->set_hash_protocol(set_bits_8, 7, 0);
#ifndef NO_CONSOLE_OUTPUT
        if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
            bmi_log->normal("BM_TRACE: ingress_pkt->group2->tcp->_PRESENT: 0x%s;  line: %d", ingress_pkt->group2->tcp->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
        }
#endif
        if ((ingress_pkt->group2->tcp->_PRESENT != 0)) {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
            }
#endif
            if (ingress_pkt->group2->tcp->_PRESENT_CUR != 1) {
#ifndef NO_CONSOLE_OUTPUT
                bmi_log->error("ERROR: %s() Header ingress_pkt->group2->tcp not present", __FUNCTION__);
#endif
            }
            set_bits_16 = ingress_pkt->group2->tcp->src_port; 
            local_bus->set_hash_src_port(set_bits_16, 15, 0);
            set_bits_16 = ingress_pkt->group2->tcp->dst_port; 
            local_bus->set_hash_dst_port(set_bits_16, 15, 0);
        } else {
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
...
#ifndef NO_CONSOLE_OUTPUT
            if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                bmi_log->normal("BM_TRACE: ingress_pkt->group2->udp->_PRESENT: 0x%s;  line: %d", ingress_pkt->group2->udp->_PRESENT.to_string(SC_HEX_US, false).c_str(), __LINE__);
            }
#endif
            if ((ingress_pkt->group2->udp->_PRESENT != 0)) {
#ifndef NO_CONSOLE_OUTPUT
                if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
                    bmi_log->normal("BM_TRACE: %s()  line: %d", __FUNCTION__, __LINE__);
                }
#endif
                if (ingress_pkt->group2->udp->_PRESENT_CUR != 1) {
#ifndef NO_CONSOLE_OUTPUT
                    bmi_log->error("ERROR: %s() Header ingress_pkt->group2->udp not present", __FUNCTION__);
#endif
                }
                set_bits_16 = ingress_pkt->group2->udp->src_port; 
                local_bus->set_hash_src_port(set_bits_16, 15, 0);
                set_bits_16 = ingress_pkt->group2->udp->dst_port; 
                local_bus->set_hash_dst_port(set_bits_16, 15, 0);
            }
        }
    }
    set_bits_8 = ((((((((((((local_bus->hash_sip(31,24) ^ local_bus->hash_sip(23,16)) ^ local_bus->hash_sip(15,8)) ^ local_bus->hash_sip(7,0)) ^ local_bus->hash_dip(31,24)) ^ local_bus->hash_dip(23,16)) ^ local_bus->hash_dip(15,8)) ^ local_bus->hash_dip(7,0)) ^ local_bus->hash_protocol(7,0)) ^ local_bus->hash_src_port(15,8)) ^ local_bus->hash_src_port(7,0)) ^ local_bus->hash_dst_port(15,8)) ^ local_bus->hash_dst_port(7,0)); 
    local_bus->set_hash_val(set_bits_8, 7, 0);
...
void _BModel::ing_main(void) {
    ...
    do_ip_hash_calc();
    printf("5-tuple hash value of the packet is: 0x%x\n", (int)local_bus->hash_val);
