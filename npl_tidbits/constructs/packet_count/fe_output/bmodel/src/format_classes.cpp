...
void local_bus_t::set_mc(_1bit value, int maxbit, int minbit) {
    bool __print__value = false;
    uint i;
...
#ifndef NO_CONSOLE_OUTPUT
    if ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_ALL) ||
        ((bm_debug_lvl>=BM_DEBUG_LVL_DEBUG_NZ) && ((mc(maxbit, minbit) != 0) or (value != 0)))) {
        __print__value = true;
    }

#endif
    mc(maxbit, minbit) = value;

#ifndef NO_CONSOLE_OUTPUT
    if (__print__value) {
        bmi_log->normal("BM_DEBUG: local_bus_t->mc: 0x" + mc.to_string(SC_HEX_US, false));
    }
#endif
}
...
void local_bus_t::print() {
    ...
    bmi_log->normal("local_bus_t->mc = 0x" + mc.to_string(SC_HEX_US, false));
#endif
}
...
int local_bus_t::compare(local_bus_t *ref, local_bus_t *ref_mask) {
    ...
    if (ref_mask == NULL) {
        ...
        if ((mc & ref_mask->mc) != (ref->mc & ref_mask->mc)) {
            mismatch = 1;
        }
    }
...
void local_bus_t::pack_data(uint32_t *_data, int offset) {
    ...
    field = (uint32_t) mc.range(0, 0).to_int();
