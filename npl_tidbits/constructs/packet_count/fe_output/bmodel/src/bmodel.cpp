...
void _BModel::do_mc_pkt_count(void) {
    _8bits pc_arg_0;
    _8bits pc_arg_1;
    _1bit set_bit_1;
...
#endif
    if (ingress_pkt->group->ether->_PRESENT_CUR != 1) {
#ifndef NO_CONSOLE_OUTPUT
        bmi_log->error("ERROR: %s() Header ingress_pkt->group->ether not present", __FUNCTION__);
#endif
    }
    set_bit_1 = ingress_pkt->group->ether->macda(40,40); 
    local_bus->set_mc(set_bit_1, 0, 0);
#ifndef NO_CONSOLE_OUTPUT
    if (bm_debug_lvl >= BM_DEBUG_LVL_TRACE) {
        bmi_log->normal("BM_TRACE: local_bus->mc: 0x%s;  line: %d", local_bus->mc.to_string(SC_HEX_US, false).c_str(), __LINE__);
    }
#endif
    if ((local_bus->mc == 1)) {
#ifndef NO_CONSOLE_OUTPUT
...
#endif
        printf("Calling packet_count.\n");
        pc_arg_0 = local_bus->mc;
        pc_arg_1 = 0x00000001;
        packet_count(&pc_arg_0, &pc_arg_1);
    }
